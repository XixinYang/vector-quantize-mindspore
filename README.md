...
This repository is a mindspore-version implementation based on the original [Vector Quantization - Pytorch](https://github.com/lucidrains/vector-quantize-pytorch), shout out to all the contributors of `Vector Quantization - Pytorch`!🤘

## Difference

Compared with `Vector Quantization - Pytorch`, here is the main difference in usage:

About **distributed training**：

This repository is written to use as a built-in tool in other models, hence no more additional code about distributed training initialization (e.g. distributed.is_initialized()) is kept, it **cannot be independently distributed trained**. However, it still **support distributed training once the outer model is trained distributedly**. 

To support distributed training, a hyper-parameter **`distribute=False`** is added to `VectorQuantize`, `RandomProjectionQuantizer`,`ResidualVQ` and `GroupedResidualVQ` to manually specify whether it is distributed training mode currently, so do not forget to take notice of it.
...
img_fmap = ops.randn((1, 256, 32, 32))
...
x = ops.randn((1, 1024, 4)) # 4 since there are 4 levels
...
assert ops.all(xhat == quantizer.indices_to_codes(indices))
```
...
assert ops.all(quantized == quantized_out)
```
...
image_feats = ops.randn((1, 16, 32, 32))
...
```python
import from mindspore import ops
...
🔘 Write example codes (e.g. autoencoder.py, etc.)  
